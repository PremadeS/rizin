...
// TODOe: verify that this correct
static int cb_default_printf(void *user, const char *fmt, ...) {
	(void)user; // Explicitly ignore user context
	va_list ap;
	va_start(ap, fmt);
...
void MACH0_(mach_headerfields)(RzBinFile *bf) {
	...
	RzCons *cons = (RzCons *)bf->rbin->cb_printf_user;
	if (!cb_printf) {
		cb_printf = cb_default_printf;
	}
	RzBuffer *buf = bf->buf;
	...
	cb_printf(cons, "pf.mach0_header @ 0x%08" PFMT64x "\n", pvaddr);
	cb_printf(cons, "0x%08" PFMT64x "  Magic       0x%x\n", pvaddr, mh->magic);
	pvaddr += 4;
	cb_printf(cons, "0x%08" PFMT64x "  CpuType     0x%x\n", pvaddr, mh->cputype);
	pvaddr += 4;
	cb_printf(cons, "0x%08" PFMT64x "  CpuSubType  0x%x\n", pvaddr, mh->cpusubtype);
	pvaddr += 4;
	cb_printf(cons, "0x%08" PFMT64x "  FileType    0x%x\n", pvaddr, mh->filetype);
	pvaddr += 4;
	cb_printf(cons, "0x%08" PFMT64x "  nCmds       %d\n", pvaddr, mh->ncmds);
	pvaddr += 4;
	cb_printf(cons, "0x%08" PFMT64x "  sizeOfCmds  %d\n", pvaddr, mh->sizeofcmds);
	pvaddr += 4;
	cb_printf(cons, "0x%08" PFMT64x "  Flags       0x%x\n", pvaddr, mh->flags);
	pvaddr += 4;
...
#define READWORD() \
	...
	for (n = 0; n < mh->ncmds; n++) {
		...
		if (pf_definition) {
			cb_printf(cons, "pf.%s @ 0x%08" PFMT64x "\n", pf_definition, pvaddr - 4);
		}
		cb_printf(cons, "0x%08" PFMT64x "  cmd %7d 0x%x %s\n",
		...
		int lcSize = word;
		word &= 0xFFFFFF;
		cb_printf(cons, "0x%08" PFMT64x "  cmdsize     %d\n", pvaddr - 4, word);
		if (lcSize < 1) {
		...
		case LC_BUILD_VERSION: {
			...
			cb_printf(cons, "0x%08" PFMT64x "  minos       %d.%d.%d\n", pvaddr + 4, minos1, minos2, minos3);
			...
			while (off < (lcSize - 8) && ntools--) {
				...
				cb_printf(cons, "0x%08" PFMT64x "  version     %d.%d.%d\n", pvaddr + off, version1, version2, version3);
		...
		case LC_MAIN: {
			...
			cb_printf(cons, "0x%08" PFMT64x "  entry0      0x%" PFMT64x "\n", pvaddr, ep);
			ut64 ss = rz_read_ble64(&data[8], big_endian);
			cb_printf(cons, "0x%08" PFMT64x "  stacksize   0x%" PFMT64x "\n", pvaddr + 8, ss);
#else
			ut32 ep = rz_read_ble32(&data, big_endian);
			cb_printf(cons, "0x%08" PFMT32x "  entry0      0x%" PFMT32x "\n", (ut32)pvaddr, ep);
			ut32 ss = rz_read_ble32(&data[4], big_endian);
			cb_printf(cons, "0x%08" PFMT32x "  stacksize   0x%" PFMT32x "\n", (ut32)pvaddr + 4, ss);
#endif
		} break;
		case LC_SYMTAB:
			break;
		case LC_ID_DYLIB: { // install_name_tool
			...
			cb_printf(cons, "0x%08" PFMT64x "  current     %d.%d.%d\n", pvaddr + 8, current1, current2, current3);
			...
			cb_printf(cons, "0x%08" PFMT64x "  compat      %d.%d.%d\n", pvaddr + 12, compat1, compat2, compat3);

			cb_printf(cons, "0x%08" PFMT64x "  id          %s\n",
		...
		case LC_UUID: {
			...
			cb_printf(cons, "0x%08" PFMT64x "  uuid        ", pvaddr);
			for (i = 0; i < sizeof(uuid); i++) {
				cb_printf(cons, "%02x", uuid[i]);
			}
			cb_printf(cons, "\n");
		} break;
		case LC_SEGMENT:
		case LC_SEGMENT_64: {
			...
			cb_printf(cons, "0x%08" PFMT64x "  name        %s\n", pvaddr, name);
			ut32 nsects;
			...
			while (off < lcSize && nsects--) {
				if (is64) {
					cb_printf(cons, "pf.mach0_section64 @ 0x%08" PFMT64x "\n", pvaddr - 8 + off);
					off += 80;
				} else {
					cb_printf(cons, "pf.mach0_section @ 0x%08" PFMT64x "\n", pvaddr - 8 + off);
					off += 68;
				}
			}
		} break;
		...
		case LC_LOAD_WEAK_DYLIB: {
			...
			cb_printf(cons, "0x%08" PFMT64x "  compat      %d.%d.%d\n", pvaddr + 12, compat1, compat2, compat3);
			cb_printf(cons, "0x%08" PFMT64x "  load_dylib  %s\n",
		...
		case LC_RPATH: {
			...
			cb_printf(cons, "0x%08" PFMT64x "  rpath       %s\n",
		...
		case LC_ENCRYPTION_INFO_64: {
			...
			cb_printf(cons, "0x%08" PFMT64x "  cryptoff   0x%08x\n", pvaddr, word);

			if (!rz_buf_read_ble32_at(buf, addr + 4, &word, big_endian)) {
				break;
			}
			cb_printf(cons, "0x%08" PFMT64x "  cryptsize  %d\n", pvaddr + 4, word);

			if (!rz_buf_read_ble32_at(buf, addr + 8, &word, big_endian)) {
				break;
			}
			cb_printf(cons, "0x%08" PFMT64x "  cryptid    %d\n", pvaddr + 8, word);
			break;
		}
		case LC_CODE_SIGNATURE: {
			ut32 words[2];
			rz_buf_read_at(buf, addr, (ut8 *)words, sizeof(words));
			cb_printf(cons, "0x%08" PFMT64x "  dataoff     0x%08x\n", pvaddr, words[0]);
			cb_printf(cons, "0x%08" PFMT64x "  datasize    %d\n", pvaddr + 4, words[1]);
			cb_printf(cons, "# wtf mach0.sign %d @ 0x%x\n", words[1], words[0]);
			break;
		}
		}
		addr += word - 8;
