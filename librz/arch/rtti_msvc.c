...
static void rtti_msvc_print_complete_object_locator(rtti_complete_object_locator *col, ut64 addr, const char *prefix, RzStrBuf *sb) {
	rz_strbuf_appendf(sb, "%sComplete Object Locator at 0x%08" PFMT64x ":\n"
			      "%s\tsignature: %#x\n"
			      "%s\tvftableOffset: %#x\n"
			      "%s\tcdOffset: %#x\n"
	...
	rz_strbuf_appendf(sb, "%s\tobjectBase: 0x%08" PFMT32x "\n\n",
...
static void rtti_msvc_print_type_descriptor(rtti_type_descriptor *td, ut64 addr, const char *prefix, RzStrBuf *sb) {
	rz_strbuf_appendf(sb, "%sType Descriptor at 0x%08" PFMT64x ":\n"
...
static void rtti_msvc_print_class_hierarchy_descriptor(rtti_class_hierarchy_descriptor *chd, ut64 addr, const char *prefix, RzStrBuf *sb) {
	rz_strbuf_appendf(sb, "%sClass Hierarchy Descriptor at 0x%08" PFMT64x ":\n"
			      "%s\tsignature: %#x\n"
			      "%s\tattributes: %#x\n"
			      "%s\tnumBaseClasses: %#x\n"
...
static void rtti_msvc_print_base_class_descriptor(rtti_base_class_descriptor *bcd, const char *prefix, RzStrBuf *sb) {
	rz_strbuf_appendf(sb, "%sBase Class Descriptor:\n"
			      "%s\ttypeDescriptorAddr: 0x%08" PFMT32x "\n"
...
RZ_API void rz_analysis_rtti_msvc_print_complete_object_locator(RVTableContext *context, ut64 addr, int mode, RZ_NONNULL RzStrBuf *sb) {
	...
	if (mode == 'j') {
		...
		rtti_msvc_print_complete_object_locator(&col, addr, "", sb);
	}
}
...
RZ_API void rz_analysis_rtti_msvc_print_type_descriptor(RVTableContext *context, ut64 addr, int mode, RZ_NONNULL RzStrBuf *sb) {
	...
	if (mode == 'j') {
		...
		rtti_msvc_print_type_descriptor(&td, addr, "", sb);
	}
...
RZ_API void rz_analysis_rtti_msvc_print_class_hierarchy_descriptor(RVTableContext *context, ut64 addr, int mode, RZ_NONNULL RzStrBuf *sb) {
	...
	if (mode == 'j') {
		...
		rtti_msvc_print_class_hierarchy_descriptor(&chd, addr, "", sb);
	}
}
...
RZ_API void rz_analysis_rtti_msvc_print_base_class_descriptor(RVTableContext *context, ut64 addr, int mode, RZ_NONNULL RzStrBuf *sb) {
	...
	if (mode == 'j') {
		...
		rtti_msvc_print_base_class_descriptor(&bcd, "", sb);
	}
}

static bool rtti_msvc_print_complete_object_locator_recurse(RVTableContext *context, ut64 atAddress, RzOutputMode mode, bool strict, RzStrBuf *sb) {
	...
	if (use_json) {
		...
		rtti_msvc_print_complete_object_locator(&col, colAddr, "", sb);
		rtti_msvc_print_type_descriptor(&td, typeDescriptorAddr, "\t", sb);
		rtti_msvc_print_class_hierarchy_descriptor(&chd, classHierarchyDescriptorAddr, "\t", sb);
	}
	...
	rz_list_foreach (baseClassArray, bcdIter, bcd) {
		...
		if (rtti_msvc_read_type_descriptor(context, baseTypeDescriptorAddr, &btd)) {
			if (use_json) {
				...
				rtti_msvc_print_type_descriptor(&btd, baseTypeDescriptorAddr, "\t\t\t", sb);
			}
...
RZ_API RZ_OWN bool rz_analysis_rtti_msvc_print_at_vtable(RVTableContext *context, ut64 addr, RzOutputMode mode, bool strict, RZ_NONNULL RzStrBuf *sb) {
	return rtti_msvc_print_complete_object_locator_recurse(context, addr, mode, strict, sb);
}
