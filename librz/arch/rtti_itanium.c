...
static void rtti_itanium_print_class_type_info(class_type_info *cti, const char *prefix, RzStrBuf *sb) {
	rz_strbuf_appendf(sb, "%sType Info at 0x%08" PFMT64x ":\n"
			      "%s  Type Info type: %s\n"
...
static void rtti_itanium_print_class_type_info_json(class_type_info *cti, RzStrBuf *sb) {
	PJ *pj = pj_new();
	if (!pj) {
		return;
	}
...
static void rtti_itanium_print_vmi_class_type_info(vmi_class_type_info *vmi_cti, const char *prefix, RzStrBuf *sb) {
	rz_strbuf_appendf(sb, "%sType Info at 0x%08" PFMT64x ":\n"
			      "%s  Type Info type: %s\n"
			      "%s  Belongs to class vtable: 0x%08" PFMT64x "\n"
			      "%s  Reference to RTTI's type class: 0x%08" PFMT64x "\n"
	...
	int i;
	for (i = 0; i < vmi_cti->vmi_base_count; i++) {
		rz_strbuf_appendf(sb, "%s    Base class type descriptor address: 0x%08" PFMT64x "\n"
...
static void rtti_itanium_print_vmi_class_type_info_json(vmi_class_type_info *vmi_cti, RzStrBuf *sb) {
	PJ *pj = pj_new();
	if (!pj) {
		return;
	}
...
static void rtti_itanium_print_si_class_type_info(si_class_type_info *si_cti, const char *prefix, RzStrBuf *sb) {
	rz_strbuf_appendf(sb, "%sType Info at 0x%08" PFMT64x ":\n"
			      "%s  Type Info type: %s\n"
			      "%s  Belongs to class vtable: 0x%08" PFMT64x "\n"
			      "%s  Reference to RTTI's type class: 0x%08" PFMT64x "\n"
...
static void rtti_itanium_print_si_class_type_info_json(si_class_type_info *si_cti, RzStrBuf *sb) {
	PJ *pj = pj_new();
	if (!pj) {
		return;
	}
...
RZ_API bool rz_analysis_rtti_itanium_print_at_vtable(RVTableContext *context, ut64 addr, RzOutputMode mode, RZ_NONNULL RzStrBuf *sb) {
	...
	case RZ_TYPEINFO_TYPE_VMI_CLASS: {
		...
		if (use_json) {
			rtti_itanium_print_vmi_class_type_info_json(vmi_cti, sb);
		} else {
			rtti_itanium_print_vmi_class_type_info(vmi_cti, "", sb);
		}
	...
	case RZ_TYPEINFO_TYPE_SI_CLASS: {
		...
		if (use_json) {
			rtti_itanium_print_si_class_type_info_json(si_cti, sb);
		} else {
			rtti_itanium_print_si_class_type_info(si_cti, "", sb);
		}
	...
	case RZ_TYPEINFO_TYPE_CLASS: {
		if (use_json) {
			rtti_itanium_print_class_type_info_json(cti, sb);
		} else {
			rtti_itanium_print_class_type_info(cti, "", sb);
		}
		rtti_itanium_class_type_info_free(cti);
	}
		return true;
