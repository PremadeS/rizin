...
static void showreg(RzCons *cons, RzAnalysisEsil *esil, const char *rn, const char *desc) {
	ut64 nm = 0;
	int sz = 0;
	rz_cons_printf(cons, "%s 0x%08" PFMT64x " (%d) ; %s\n", rn, nm, sz, desc);
}
...
RZ_IPI bool rz_core_visual_esil(RzCore *core) {
	...
	for (;;) {
		...
		rz_cons_printf(cons, "rizin's esil debugger:\n\n");
		rz_cons_printf(cons, "pos: %d\n", x);
		{
			char *op_hex = rz_asm_op_get_hex(&asmop);
			char *res = rz_print_hexpair(core->print, op_hex, -1);
			rz_cons_printf(cons, "hex: %s\n" Color_RESET, res);
			free(res);
			free(op_hex);
		}
		{
		...
		}
		{
			...
			rz_cons_printf(cons, Color_RESET "      %s%s\n" Color_RESET, pas, pad);
			free(pas);
			free(pad);
		}
		rz_cons_printf(cons, "esil regs:\n");
		showreg(cons, esil, "$$", "address");
		showreg(cons, esil, "$z", "zero");
		showreg(cons, esil, "$b", "borrow");
		showreg(cons, esil, "$c", "carry");
		showreg(cons, esil, "$o", "overflow");
		showreg(cons, esil, "$p", "parity");
		showreg(cons, esil, "$r", "regsize");
		showreg(cons, esil, "$s", "sign");
		showreg(cons, esil, "$d", "delay");
		showreg(cons, esil, "$j", "jump");

		rz_cons_printf(cons, "regs:\n");
		char *r = rz_core_cmd_str(core, "dr=");
		if (r) {
			rz_cons_printf(cons, "%s", r);
			free(r);
		}
		rz_cons_printf(cons, "esil stack:\n");
		rz_core_esil_dumpstack(cons, esil);
		...
		ch = rz_cons_arrow_to_hjkl(cons, ch); // get ESC+char, return 'hjkl' char
		switch (ch) {
		case 'Q':
		case 'q':
			goto beach;
