...
RZ_API bool rz_core_bin_apply_strings(RzCore *r, RzBinFile *binfile) {
	...
	rz_cons_break_push(r->cons, NULL, NULL);
	void **iter;
	RzBinString *string;
...
RZ_API void rz_core_bin_print_source_line_info(RzCore *core, const RzBinSourceLineInfo *li, RzCmdStateOutput *state) {
	...
	rz_cons_break_push(core->cons, NULL, NULL);
	for (size_t i = 0; i < li->samples_count; i++) {
		if (rz_cons_is_breaked(core->cons)) {
			break;
		}
...
// TODO: check whether this is fine or no
static bool core_basefind_progess_status(const RzBaseFindThreadInfo *th_info, void *user) {
	...
	return !rz_cons_is_breaked(core->cons);
}
...
RZ_IPI RzCmdStatus rz_core_bin_plugin_print(RZ_NONNULL RzCore *core, const RzBinPlugin *bp, RzCmdStateOutput *state) {
	...
	case RZ_OUTPUT_MODE_JSON:
		...
		break;
	case RZ_OUTPUT_MODE_STANDARD:
		rz_cons_printf(core->cons, "%-12s %-9s %-15s %-38s %s\n", bp->name,
...
RZ_IPI RzCmdStatus rz_core_binxtr_plugin_print(RZ_NONNULL RzCore *core, const RzBinXtrPlugin *bx, RzCmdStateOutput *state) {
	...
	switch (state->mode) {
	case RZ_OUTPUT_MODE_QUIET:
		rz_cons_println(core->cons, bx->name);
		break;
	case RZ_OUTPUT_MODE_JSON:
		pj_o(state->d.pj);
	...
	case RZ_OUTPUT_MODE_STANDARD:
		...
		rz_cons_printf(core->cons, "%-12s %-9s %-15s %s\n", name,
...
RZ_API RzCmdStatus rz_core_bin_plugins_print(RZ_NONNULL RzCore *core, RzBin *bin, RzCmdStateOutput *state) {
	...
	rz_list_foreach (plugin_list, it, bp) {
		status = rz_core_bin_plugin_print(core, bp, state);
	...
	rz_list_foreach (plugin_list, it, bx) {
		status = rz_core_binxtr_plugin_print(core, bx, state);
		if (status != RZ_CMD_STATUS_OK) {
			rz_iterator_free(iter);
			rz_list_free(plugin_list);
			return status;
...
// TODO: the RZ_NONNULL type shi --- check everywhere
