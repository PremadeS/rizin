		...
		if (op->il_op) {
			...
			if (RZ_STR_ISNOTEMPTY(ilstr)) {
				if (use_color) {
                    ...
                    rz_core_il_colorize_body(core->cons, core->cons->context, ilstr);
...
static void showregs(RzList /*<, core->conschar *>*/ *list, RzCons* cons) {
	if (!rz_list_empty(list)) {
		char *reg;
		RzListIter *iter;
			...
			while (pc < end) {
				...
				if (ret > 0) {
					if (opskip) {
                        ...
                        rz_core_esil_dumpstack(core->cons, esil);
						rz_analysis_esil_stack_free(esil);
					}
					pc += op.size;
				} else {
...
RZ_IPI RzCmdStatus rz_analysis_list_vtables_handler(RzCore *core, int argc, const char **argv, RzOutputMode mode) {
    ...
    rz_analysis_list_vtables(core->analysis, mode, sb);
...
RZ_IPI RzCmdStatus rz_analysis_print_rtti_handler(RzCore *core, int argc, const char **argv, RzOutputMode mode) {   
    RzStrBuf* sb = rz_strbuf_new("");
    rz_analysis_rtti_print_at_vtable(core->analysis, core->offset, mode, sb);
    char *s = rz_strbuf_drain(sb);
    rz_cons_print(core->cons, s);
    free(s);
    return RZ_CMD_STATUS_OK;
...
RZ_IPI RzCmdStatus rz_analysis_print_rtti_all_handler(RzCore *core, int argc, const char **argv, RzOutputMode mode) {
    RzStrBuf* sb = rz_strbuf_new("");
    rz_analysis_rtti_print_all(core->analysis, mode, sb);
    char *s = rz_strbuf_drain(sb);
    rz_cons_print(core->cons, s);
    free(s);
	return RZ_CMD_STATUS_OK;
...
RZ_IPI RzCmdStatus rz_analysis_hint_list_handler(RzCore *core, int argc, const char **argv, RzCmdStateOutput *state) {
    rz_core_analysis_hint_list_print(core->cons, core->analysis, state);
	return RZ_CMD_STATUS_OK;
}

RZ_IPI RzCmdStatus rz_analysis_hint_list_at_handler(RzCore *core, int argc, const char **argv, RzCmdStateOutput *state) {
    rz_core_analysis_hint_print(core->cons, core->analysis, core->offset, state);
	return RZ_CMD_STATUS_OK;
}
