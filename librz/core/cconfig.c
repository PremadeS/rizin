...
static bool cb_cfg_fortunes_file(void *user, void *data) {
	RzCore *core = (RzCore *)user; // TODOe: verify this is set as core
	RzConfigNode *node = (RzConfigNode *)data;
	if (RZ_STR_EQ(node->value, "?")) {
 ...
 *
 * \param core reference to RzCore TODOe: update prev docs to match this style
...
static bool cb_graphformat(void *user, void *data) {
	RzConfigNode *node = (RzConfigNode *)data;
	RzCore *core = (RzCore *)user; // TODOe: verify this and everything else to make sure *user is indeed core
	if (RZ_STR_EQ(node->value, "?")) {
