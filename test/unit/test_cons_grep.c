...
bool test_grep_parse_simple(void) {
	...
	rz_cons_grep_parsecmd(cons, cmd, "");
...
bool test_grep_line(void) {
	RzCons *cons = rz_cons_new();
	rz_cons_grep(cons, "mov");
	char line[] = "mov eax, 1";
	int len = rz_cons_grep_line(cons, line, strlen(line));
	mu_assert_eq(len, 10, "Grep line matches");

	char line2[] = "sub eax, 1";
	int len2 = rz_cons_grep_line(cons, line2, strlen(line2));
...
bool test_grep_json_path(void) {
	...
	for (size_t i = 0; i < RZ_ARRAY_SIZE(cases); i++) {
		RzCons *cons = rz_cons_new();
		rz_cons_print(cons, cases[i].json);
		rz_cons_grep(cons, cases[i].grep);
		mu_assert_streq(rz_cons_get_buffer(cons), cases[i].expected, cases[i].message);
		rz_cons_free(cons);
	}
	mu_end;
}
